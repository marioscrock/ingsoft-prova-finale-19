...
public class SingleButtonSceneManager extends Observable implements EventHandler<ActionEvent> {
    ...
    public Scene create() {
        ...
        value = new Text();
        //Use a single class from CSS
        value.getStyleClass().add("scene-text"); // for CSS
        refreshText();

        button = new Button(name + " Action");
        goTo = new Button("Change Mod");
     ...
     * It holds a fx.snapshot of the current state.
     * @param event
     */
    @Override
    public void handle(ActionEvent event) {
        try {
            ...
            Parent root = FXMLLoader.load(getClass().getResource("/fx.snapshot.fxml"));
